...
// A bridge-level fault (node-osc missing, second device copy on port 9001)
...
try {
  supervisor = require("./backend_supervisor.js").createSupervisor({
    ...
    // The supervisor cannot own a socket (Python replies to OUR port 9001),
...
const voicingOptions = {
  ...
  colorMinor: 0,
  color7th: 0,
  // Voicing dial (functional ladder) + Voice-Distance dial state. Defaults
  // are the plain-triad behaviour, so chord_parser voices exactly as before
  // until these are driven. See performance_map.js and chord_parser.js.
...
function emit(address, args) {
  ...
  if (address === "/status/model") {
    // Python's CONFIRMED active model (markov/rnn/lstm).
...
function initOsc() {
  ...
  server.on("error", (err) => {
    ...
    if (err && err.code === "EADDRINUSE") {
      // A second copy of the device already owns Max's reply port: only one
      // instance per Set can talk to the backend on the fixed 9000/9001 pair.
...
// Beat grid. Straight = 4 ticks/bar (quarter notes, TEMPLATES onsets 0..3).
// Triplet = 6 ticks/bar (quarter-note triplets); the clock in the patch switches
// the metro to 4nt to match, and chords fall on `tripletOnsets` within each bar.
let ticksPerBar = 4;
let tripletOnsets = null; // e.g. [0,2,4] => 3 chords/bar; null => use straight templates
...
/** Does a chord change land on this absolute beat (respecting triplet mode)? */
 ...
 * Durations arrive in BEATS; a bar is 4 beats but `ticksPerBar` ticks, so
 * triplet mode maps 4 beats onto 6 ticks.
 * --------------------------------------------------------------------- */
 ...
 * selection without typing. See performance_map.js for the lists.             */
...
/** Generative model selector (markov / rnn / lstm) — forwarded to Python. */
 ...
 *  quarter-note triplets (6/bar, 3 chords per bar). The patch switches the metro
 *  to 4nt in parallel so the node's tick count and the clock stay in step. */
Max.addHandler("triplet", (v) => {
  const on = Number(v) !== 0;
  ticksPerBar = on ? 6 : 4;
  tripletOnsets = on ? [0, 2, 4] : null;
  player.dirty = true; // re-capture on the next cycle so the grid change is clean
  // The phrase engine's durations are in BEATS, so the installed phrase is
  // still valid — only its tick positions move. Re-lay it now rather than
  // playing the rest of this cycle against the wrong grid.
 ...
 * (control-change outlet). All PC/CC meaning lives in performance_map.js.    */
