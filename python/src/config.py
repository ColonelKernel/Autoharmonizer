...
# Phrase generation (v3, additive): the SEQUENCE is the unit of analysis.
...
OSC_CONTROL_MODEL = "/control/model"          # string: markov | rnn | lstm
OSC_CONTROL_GRAVITY = "/control/gravity"      # float 0..1 harmonic gravity (cadence pull)
# Neural session control (v3): string auto | stateless | session | reset
OSC_CONTROL_SESSION = "/control/session"

# Generative model backends. All three are implemented: "markov" is the
# corpus-blend chain; "rnn"/"lstm" are the JazzNet neural engines (loaded on
# demand). MODEL_NAMES is an alias kept for parity with the neural codebase.
MODELS = ("markov", "rnn", "lstm")
MODEL_NAMES = MODELS
...
# --- Neural (RNN/LSTM) sampling + session (protocol v3) ------------------------
...
class Settings:
    ...
    # Neural + session (protocol v3). Defaulted so existing callers that build
