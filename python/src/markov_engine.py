...
class MarkovEngine:
    ...
    def sample(self, raw_input: str) -> SampleResult:
        ...
        if self._corpora is not None:
            return self._blend_sample(chord)
        return self._legacy_sample(chord)
    ...
    def _blend_sample(self, chord: str) -> SampleResult:
        ...
        if choices:
            chosen_norm, prob = self._choose(choices)
            return SampleResult(
                output=transpose_chord(chosen_norm, -offset),
                probability=prob,
                candidates=len(choices),
                fallback_used=False,
                mix=mix,
            )
        return self._blend_fallback(chord, norm_in, offset, tau, mix)
    ...
    def _blend_fallback(
        ...
        if policy == "random_source":
            ...
            if sources:
                ...
                chosen_norm, prob = self._choose(choices)
                return SampleResult(
                    transpose_chord(chosen_norm, -offset),
                    prob,
                    len(choices),
                    True,
                    error=error,
    ...
    def _legacy_sample(self, chord: str) -> SampleResult:
        weights = self._table.weighted_choices_by_source.get(chord)
        if weights:
            chosen, prob = self._choose(weights)
    ...
    def _apply_fallback(self, chord: str) -> SampleResult:
        ...
        if policy == "random_source":
            ...
            chosen, prob = self._choose(self._table.weighted_choices_by_source[source])
