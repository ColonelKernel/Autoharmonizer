...
from ..harmony.theory import chord_complexity_tier, complexity_level
...
class PhraseEngine:
    ...
    def __init__(self, model_path: str | Path, seed: int | None = None) -> None:
        ...
        self._complexity = DEFAULT_COMPLEXITY
    ...
    def set_complexity(self, value: float) -> None:
        try:
            self._complexity = max(0.0, min(1.0, float(value)))
    ...
    def _draw_motion(self, quality: str, rng: random.Random, root: int = 0,
        ...
        if ctx and abs(float(ctx.get("complexity", DEFAULT_COMPLEXITY)) - DEFAULT_COMPLEXITY) > 1e-9:
            level = complexity_level(ctx["complexity"])
            key = str(ctx.get("key", "C:maj"))
            classified = [
                (k, p, chord_complexity_tier(
                    f"{CANON_ROOT[(root + int(k[0])) % 12]}:{str(k[1])}", key
                ))
                for k, p in dist
            ]
            permitted = [(k, p, tier) for k, p, tier in classified if tier <= level]
            if not permitted:
                minimum = min((tier for _, _, tier in classified), default=4)
                permitted = [(k, p, tier) for k, p, tier in classified if tier == minimum]
            target = float(ctx["complexity"]) * 4.0
            weighted = [[k, p * (1.0 / (1.0 + abs(tier - target)))] for k, p, tier in permitted]
            total = sum(p for _, p in weighted)
            if total > 0:
                dist = [[k, p / total] for k, p in weighted]
        pick = _pick(rng, dist)
    ...
    def generate(self, bars: int, key: str, seed_chord: str | None = None,
        ...
        ctx = {"tonic": tonic_pc, "mode": mode, "gravity": gravity,
               ...
               "complexity": self._complexity, "key": key}
        ...
        if not resolve or total < 4 + 2:
            ...
            return self._realize(
                self._render(self._body(start_root, start_q, total, rng, ctx)), key, seed
            )
        ...
        return self._realize(self._render(segs), key, seed)

    def _realize(
        self, phrase: list[tuple[str, int]], key: str, seed: int | None
    ) -> list[tuple[str, int]]:
        ...
        for symbol, duration in phrase:
            realized = planner.realize(symbol)
            if out and out[-1][0] == realized:
                out[-1] = (realized, out[-1][1] + duration)
            else:
                out.append((realized, duration))
        return out
