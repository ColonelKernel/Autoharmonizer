"""JazzNet baseline RNN chord engine."""
...
class RnnEngine:
    ...
    def _ensure_loaded(self) -> None:
        ...
        checkpoint = (
            self._jazznet_dir / "checkpoints" / "rnn" / f"baselineRNN-epoch{self._epoch}.pt"
        )
        if not checkpoint.is_file():
            raise FileNotFoundError(f"RNN checkpoint not found: {checkpoint}")
        ...
        self._model = model
        logger.info("loaded RNN checkpoint epoch=%s from %s", self._epoch, checkpoint)
    ...
    def sample(self, raw_input: str, *, session: bool = False, candidate_selector=None) -> SampleResult:
        ...
        try:
            ...
            return SampleResult(
                ...
                candidates=0,
                fallback_used=True,
                error=f"RNN engine unavailable: {exc}",
            )
