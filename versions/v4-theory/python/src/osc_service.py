...
class MarkovOscService:
    ...
    def load_table(self) -> TransitionTable:
        ...
        gravity = prev._gravity if prev is not None else self._settings.gravity
        ...
        if registry is None:
            registry = EngineRegistry(
                ...
                ngram_model_path=self._settings.ngram_model_path,
                key=key,
                complexity=self._settings.complexity,
                gravity=gravity,
            )
        else:
        ...
        if self._settings.phrase_model_path is not None:
            try:
                phrase = PhraseEngine(self._settings.phrase_model_path,
                                      seed=self._settings.seed)
                phrase.set_complexity(registry.complexity)
    ...
    def _handle_ping(self, _address: str, *_args: object) -> None:
        ...
        self._emit_protocol_status()
        self._emit_complexity_status()
    ...
    def _emit_protocol_status(self) -> None:
        self._send(OSC_STATUS_PROTOCOL, PROTOCOL_VERSION)

    def _emit_complexity_status(self) -> None:
        try:
            self._send(OSC_STATUS_COMPLEXITY, float(self._get_registry().complexity))
        except RuntimeError:
            pass
    ...
    def _handle_gravity(self, address: str, *args: object) -> None:
        self._control_float(
            address, args, lambda _e, v: self._get_registry().set_gravity(v)
        )

    def _handle_complexity(self, address: str, *args: object) -> None:
        def _set(_engine, value: float) -> None:
            self._get_registry().set_complexity(value)
            phrase = self._phrase
            if phrase is not None:
                phrase.set_complexity(value)

        self._control_float(address, args, _set)
        self._emit_complexity_status()
    ...
    def _handle_key(self, _address: str, *args: object) -> None:
        if not args or not isinstance(args[0], str) or not args[0].strip():
            ...
            return
        self._get_registry().set_key(args[0].strip())
    ...
    def _build_dispatcher(self) -> Dispatcher:
        ...
        dispatcher.map(OSC_CONTROL_COMPLEXITY, self._handle_complexity)
